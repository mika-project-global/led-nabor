/*
  # Исправление проблем с миграцией и функциями управления ценами
  
  1. Изменения
    - Удаление конфликтующих функций
    - Исправление функций для работы с вариантами продуктов
...
DROP FUNCTION IF EXISTS get_warranty_price_by_variant(integer, integer, text, text);
DROP FUNCTION IF EXISTS get_all_warranty_prices_by_variant(text, text);
DROP FUNCTION IF EXISTS update_warranty_price_by_variant(integer, integer, text, numeric, text);
DROP FUNCTION IF EXISTS update_warranty_prices_for_all_variants(integer, integer, text, jsonb);
DROP FUNCTION IF EXISTS update_warranty_prices_for_all_variants_from_base(integer, integer, text, numeric);
DROP FUNCTION IF EXISTS calculate_warranty_price_for_variant(numeric, text);
DROP FUNCTION IF EXISTS get_product_warranty_prices(integer, text, text);
DROP FUNCTION IF EXISTS get_warranty_price_with_fallback(integer, integer, text);
DROP FUNCTION IF EXISTS get_warranty_price_with_fallback(integer, integer, text, text);
DROP FUNCTION IF EXISTS get_product_price_with_fallback(integer, text, text);
DROP FUNCTION IF EXISTS get_exact_warranty_price(integer, integer, text);
...
-- Создаем функцию для получения цен продуктов
...
BEGIN
  ...
  -- Если нет результатов, синхронизируем из таблицы products
  IF NOT FOUND THEN
    -- Вставляем цены из таблицы products
    INSERT INTO product_prices (
      product_id,
      variant_id,
      currency,
...
BEGIN
  ...
  -- Создаем уникальное имя для временной таблицы
  temp_table_name := 'temp_fixed_prices_' || to_char(now(), 'YYYYMMDD_HH24MISS');
  
  -- Обновляем warranty_fixed_prices из warranty_custom_prices
  -- Сначала собираем все уникальные записи во временную таблицу
  EXECUTE 'CREATE TEMP TABLE ' || temp_table_name || ' AS
  ...
  EXECUTE 'INSERT INTO warranty_fixed_prices (
    product_id,
    months,
    currency,
    price
