...
-- Create function to get warranty prices with variant
...
-- Create function to get product price
...
DECLARE
  ...
  base_price numeric;
BEGIN
  -- First try to get price from product_prices
  ...
  -- If not found, try to get from products
...
-- Create function to update product price
...
BEGIN
  ...
  -- If updating CZK price, also update products table
  IF p_currency = 'CZK' THEN
    -- Find variant index, length and stock status
    SELECT 
    ...
    -- Update products table if variant found
...
-- Create function to get warranty price
...
BEGIN
  ...
  -- First try to get price from warranty_custom_prices
  ...
  -- If not found, try to get from warranty_fixed_prices
  ...
  -- If not found, try to get from warranty_policies
...
CREATE FUNCTION get_warranty_price_for_variant(
  p_product_id integer,
  p_months integer,
  p_currency text,
  p_variant_id text
...
-- Create function to update warranty price with variant
...
BEGIN
  ...
  -- First collect all unique records into a temporary table
  ...
  -- Then update warranty_fixed_prices
...
CREATE FUNCTION debug_warranty_price(
  p_product_id integer,
  p_months integer,
  p_currency text DEFAULT 'CZK',
  p_variant_id text DEFAULT 'rgb-5'
...
CREATE FUNCTION debug_product_price(
  p_product_id integer,
  p_variant_id text,
  p_currency text DEFAULT 'CZK'
) RETURNS TABLE (
