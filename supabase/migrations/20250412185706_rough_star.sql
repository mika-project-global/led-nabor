/*
  # Implement warranty price scaling based on product length
  
  1. Changes
    - Update get_warranty_price_for_variant function to automatically scale prices
    - Ensure prices are correctly multiplied based on variant length
...
-- Create improved function to get warranty price for specific variant with automatic scaling
...
BEGIN
  ...
  IF p_variant_id != 'rgb-5' THEN
    ...
    IF default_price IS NOT NULL THEN
      -- Simple 2x scaling for each variant length
...
-- Create function to update warranty prices with proper variant handling
  ...
  LOOP
    ...
    IF base_price > 0 THEN
      ...
      LOOP
        ...
        -- Calculate price based on length ratio - simple 2x scaling
        IF v_id = 'rgb-5' THEN
