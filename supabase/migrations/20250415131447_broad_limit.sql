/*
  # Исправление цен гарантий для разных метражей
  
  1. Изменения
    - Исправление функций для корректного отображения цен гарантий для всех метражей
    - Добавление поддержки CCT вариантов продуктов
    - Обновление функции для получения цен гарантий с учетом метража
    - Обновление функции для обновления цен гарантий с учетом метража
...
BEGIN
  ...
  IF p_variant_id = product_prefix || '-5' THEN
    ...
    LOOP
      -- Пропускаем базовый вариант, так как мы уже обновили его
...
BEGIN
  ...
  IF p_currency = 'CZK' THEN
    -- Находим индекс варианта и длину
    SELECT 
        ...
        variants = jsonb_set(
          ...
          jsonb_build_object(
            ...
            'stockStatus', COALESCE(
              (SELECT value->>'stockStatus' FROM jsonb_array_elements(variants) AS value WHERE value->>'id' = p_variant_id),
              'in_stock'
            )
          )
        ),
